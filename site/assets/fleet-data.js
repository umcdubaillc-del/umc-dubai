...
const DEFAULT_FLEET = [
  ...
  {id:"mb-e-class",marque:"/assets/marques/mercedes.png",name:"Mercedes Benz E Class",category:"Business Sedan",year:2025,seats:3,luggage:2,
  ...
  {id:"lexus-es",marque:"/assets/marques/lexus.jpg",name:"Lexus ES",category:"Business Sedan",year:2024,seats:3,luggage:2,
...
// P-1: intrinsic dims for the marque logos so the client-rendered cards carry
// explicit width/height (matches .marque{height:24px;width:auto} aspect — no
// visual change, guards CLS, clears the Lighthouse flag).
const UMC_MARQUE_DIMS = {
  "mercedes.png":[553,451], "bmw.png":[501,498], "cadillac.jpg":[2560,1440],
  "gmc.png":[502,497], "lexus.jpg":[638,420], "king-long.png":[400,248]
};
function marqueImg(src){
  if(!src) return "";
  const d = UMC_MARQUE_DIMS[String(src).split("/").pop()];
  const dim = d ? ` width="${d[0]}" height="${d[1]}"` : "";
  return `<img class="marque" src="${esc(src)}" alt=""${dim} loading="lazy">`;
}
